...
- **Capability-aware routing + CLI family gate:** added [`scripts/routing_table.py`](scripts/routing_table.py) as a role-to-engine advisory table used by the semantic router as a tie-break nudge, and tightened `ptme.py` so model families are accepted only when the target engine's CLI capability allows them (`claude -> claude`, `codex -> gpt`, `agy -> gemini|claude|gpt-oss`). Unknown model slugs now fail closed back to the engine-native default.
- **Honest per-engine usage labels:** `scripts/usage_bridge/usage_bridge_reader.py` now prefers real exported agy `/usage` groups when present, keeps codex quota windows and claude token totals as-is, and always returns a labeled best-effort reading instead of an unlabeled placeholder.
