#!/usr/bin/env bash
#
# deploy/vps/install_clis.sh — install the three agent CLIs for the service account on Ubuntu.
#
# OPTIONAL (CLI-default path). Installs, as the non-root `aoa` user:
...
# the `aoa` user on the box (see docs/self-hosting-vps.md).
#
...
#
# Run as root (sudo); it drops to `aoa` for the per-user installs:
...
echo "[clis] 2/5 ensuring non-root '${SERVICE_USER}' user exists (with a login shell)…"
# The service account needs a real shell for interactive CLI login:
...
# live in the service user's home (keeps installs non-root, on PATH for that user).
...
echo "[clis] 4/5 installing Claude Code + Codex CLI (npm, as ${SERVICE_USER})…"
sudo -u "$SERVICE_USER" bash -lc '
    set -e
...
echo "[clis] 5/5 installing Antigravity CLI (agy, as ${SERVICE_USER})…"
...
'
echo
echo "[clis] NEXT: log each CLI in ONCE, interactively, as the service user:"
echo "  sudo -iu ${SERVICE_USER}"
